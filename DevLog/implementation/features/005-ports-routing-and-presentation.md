...
2. **Splitters, underground crossings, bridges, power poles.** Need new sprites/palette entries and are research-gated in [007](../../design/007-technology-tree-design.md); implementing them now would preempt the research economy. `production-data.ts` already defines `splitter`, `underground`, and `power-pole` entries.
