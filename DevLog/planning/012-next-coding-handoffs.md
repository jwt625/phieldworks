...
> File-level tranche-A handoff: [019–022 execution package](tranche-a/019-tranche-a-plan.md), including cached references and JSON task/asset manifests. Follow that package for the first precision cell; the older queue below remains historical.

> Planning addition: [018](../design/018-fusion-energy-roadmap.md) and the D-F1/D-F2 extensions in [017](017-industrial-gameplay-tranches.md) add experimental fusion and sustained net energy. Share infrastructure/research across manufacturing, energy and propulsion. Final launch requires sustained fusion supply; conventional power bootstraps it.

> Current planning authority: [016 — coherence, industry and lightsail design](../design/016-coherence-industry-and-lightsail-design.md) and [017 — implementation tranches](017-industrial-gameplay-tranches.md), approved for documentation on 2026-09-13. The A–E sequence in 017 supersedes the future P1–P5 ordering below. Retain the historical implementation evidence and applicable accounting/verification contracts. Start with local targets/controllers/qualification and the first precision cell; research follows that working process. Remaining P0.1 performance work maps to tranche B and can be investigated alongside A. This update does not mark new mechanics implemented.
...
- A-01 (local records + save v4) is implemented. `World` is now `version:4` with explicit target/reference/domain/qualification records and a bounded process inventory; the writable `target`/`controller`/`commission` globals and the façade↔persistence import cycle are removed. Loading a qualified certificate now marks it `stale`. `npm test` 66 pass, `npm run build` passes, `npm run test:browser` 19/19. Details and remaining gaps in [021](tranche-a/021-tranche-a-coding-tasks.md) and the [journal](../implementation/004-implementation-journal.md). A-02 is next; it makes emitter assignment authoritative, which A-01 deliberately deferred.
- A-02 (conserved target delivery) is implemented. `targets.ts` holds the frontier single-mode and process two-mode projections; `evaluate` routes each powered emitter through its `target.emitters` assignment and reports `stats.targets`/`protectiveAbsorption`. Process workpiece heat and exposure gating are still A-04/A-05. `npm test` 75 pass, build passes, browser 19/19.
- A-03 (local control and qualification) is implemented. `control.ts` schedules one tuner per world step across enabled domains; `qualification.ts` owns dependency closures, configuration signatures and live operating codes, and `invalidate` is now signature-based so unrelated repairs do not fail other domains. Process qualification remains A-04/A-05. `npm test` 90 pass, build passes, browser 19/19.
- A-04 (process lifecycle) is implemented. `process.ts`/`process-recipes.ts` own the versioned standard cycle, atomic reservation, exposure/suspension, terminal commit and one rework; `World` carries `jobs`/`eventSeq`; loss via cancellation, dismantling and destruction shares one helper. Process qualification events and cell thermal absorption remain A-05. `npm test` 104 pass, build passes, browser 19/19.
- A-05 (buildable first cell) is implemented. `fabrication-cell` is buildable with a labeled placeholder, owns its target/domain/qualification, absorbs process power into the declared cooling law, and a no-injection fixture reaches the frontier, mines crystal, builds the cell and completes three accepted qualified cycles. Cell art is still unapproved placeholder work (A-09/A-11). `npm test` 107 pass, build passes, browser 19/19.
- A-06 (contextual workflow) is implemented. Process/domain inspectors expose assignment, manual phase, Run once/continuous, Auto tune, test/cancel and a textual dose forecast; diagnostics deep-link to the owning cell; the tutorial has a skippable first-cell lesson. `npm test` 107 pass, build passes, browser 21/21. Full keyboard placement remains A-08.
- A-07 (module capture and second cell) is implemented. `blueprints.ts` captures a selection (or the outpost) as a version-2 template with stripped runtime state, fresh internal keys, fresh coherence groups and external power/reference/controller slots; placement is transactional with id/stock rollback; a `Select module` tool renders the selection and excluded crossing routes. `npm test` 115 pass, build passes, browser 22/22.
- A-08 (menu shell and keyboard placement) is implemented. `PauseState` owns independent user/hidden/modal reasons; Esc opens a session menu (or cancels build/route), operations lists domains/qualification/modules plus the read-only research preview, the palette has search, and a keyboard placement cursor (arrows/Enter/R/Esc) works alongside the mouse. `npm test` 115 pass, build passes, browser 26/26. Remaining A-09–A-11 are gated cell art; A-12 is integrated verification/playtest.
- A-12 (automated completion) is implemented. Fresh-expedition browser coverage manages two independent cells; same-machine two-cell benchmark and connected/solver evidence are under `DevLog/evidence/tranche-a/`. `npm test` 115 pass, build passes, browser 27/27. Visual review and a human playtest remain open, and cell art is still an unapproved placeholder.
