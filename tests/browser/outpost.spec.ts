...
test('first outpost can be built, tuned, commissioned, saved and reset through UI',async({page})=>{
 ...
 await page.locator('#objective-toggle').click();await page.locator('#controller').check();await page.locator('#speed').click();await page.locator('#speed').click();await expect.poll(async()=>(await snapshot(page)).frontier,{timeout:45000}).toBe(true);
 ...
 await page.getByRole('button',{name:'Pause simulation',exact:true}).click();const saved=await snapshot(page);await page.locator('#menu').click();await page.locator('#session-save').click();await page.locator('#menu').click();await page.locator('#session-new').click();await page.locator('#confirm-new').click();expect((await snapshot(page)).frontier).toBe(false);await page.locator('#menu').click();await page.locator('#session-load').click();const loaded=await snapshot(page);expect(loaded.frontier).toBe(true);expect(loaded.entities).toHaveLength(saved.entities.length);expect(loaded.qualifications[0].status).toBe('stale');expect(loaded.blueprint).not.toBeNull();expect(errors).toEqual([]);
});
test('invalid placement is non-destructive, route disconnection works, and manual is usable',async({page})=>{await ready(page);await page.getByRole('button',{name:'Pause simulation',exact:true}).click();const before=await snapshot(page);await page.getByRole('button',{name:'Build Field emitter',exact:true}).click();await point(page,10.1,9.1);await expect(page.locator('#toast')).toContainText('occupied');expect((await snapshot(page)).stock.assemblies).toBe(before.stock.assemblies);await page.keyboard.press('Escape');await point(page,10.5,9.5);await page.locator('details[data-group=ports] summary').click();await page.locator('[data-disconnect]').first().click();expect((await snapshot(page)).links).toHaveLength(before.links.length-1);await page.locator('#menu').click();await page.locator('#session-manual').click();await expect(page.getByRole('dialog')).toBeVisible();await page.locator('#start-playing').click();await expect(page.getByRole('dialog')).toBeHidden();await page.setViewportSize({width:1024,height:768});await page.screenshot({path:'test-results/compact-outpost.png',fullPage:true});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);});
...
test('rotation, diagonal waypoints, bend profiles and material ports are usable',async({page})=>{
 ...
 await page.locator('#tool-select').click();await point(page,4.5,6);state=await snapshot(page);const belt=state.links.find((l:any)=>l.type==='material');await page.locator('details[data-group=ports] summary').click();await page.locator(`[data-disconnect="${belt.id}"]`).click();
 ...
 await page.locator('#menu').click();await page.locator('#session-save').click();await page.locator('#menu').click();await page.locator('#session-load').click();state=await snapshot(page);expect(state.entities.find((e:any)=>e.id===tuner.id).rotation).toBe(1);expect(state.links.find((l:any)=>l.id===route.id).path).toEqual(route.path);
