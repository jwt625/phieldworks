...
const review=JSON.parse(readFileSync('assets/technology/art-review-status.json','utf8')).entries;
...
const entries=[];
for(const initial of pass.jobs){const j={...initial,path:review[initial.id]?.path??initial.path};if(!existsSync(j.path)){entries.push({id:j.id,path:j.path,status:'pending'});continue;}
