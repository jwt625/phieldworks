...
export interface EquipmentState {condition:Condition;work:WorkState;powered:boolean;passive:boolean;field:boolean;moving:boolean;hot:boolean;smoke:boolean;label:string;damageLabel:string;shotAge?:number}
...
export function equipmentState(w:World,e:Entity):EquipmentState {
 ...
 let work:WorkState='ready',moving=false,shotAge:number|undefined;
 ...
 else if(e.kind==='sentry'){const p=center(e),shots=w.ecology.shots.filter(s=>s.ttl>0&&Math.hypot(s.from.x-p.x,s.from.y-p.y)<.01);moving=shots.length>0;shotAge=moving?Math.max(...shots.map(s=>Math.max(0,.12-s.ttl))):undefined;work=moving?'firing':'ready';}
 ...
 return {condition,work,powered:e.powered&&!e.tripped&&condition!==3,passive,field:field&&condition!==3,moving,hot:e.temperature>65&&condition!==3,smoke:e.temperature>105&&condition!==3,label:labels[work],damageLabel:condition===1?'LIGHT DAMAGE':condition===2?'SEVERE DAMAGE':condition===3?'DESTROYED':'',shotAge};
}
