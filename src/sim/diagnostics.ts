...
export function diagnose(w:World):Issue[]{
 ...
 for(const domain of w.domains){const qualification=w.qualifications.find(q=>q.domain===domain.id),owner=w.targets.find(t=>t.id===domain.target)?.owner??domain.id;
  if(!domain.reference)add(owner,'Control domain has no reference','Assign a powered reference station before tuning or testing.');
  else if(!referenceReady(w,domain))add(owner,'Reference unavailable','Power and repair the reference station for this domain.');
  if(qualification&&(qualification.status==='failed'||qualification.status==='stale'))add(owner,qualification.status==='stale'?'Qualification is stale':'Qualification failed',`${qualification.reason}${qualification.code?` (${qualification.code})`:''}`);
 }
